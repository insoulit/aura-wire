...
@php
    ...
    $baseClasses = "group flex items-center justify-between w-full {$sizeClasses} font-medium rounded-lg transition-colors cursor-pointer select-none";
    ...
    <a href="{{ $href }}" {{ $attributes->merge(['class' => "{$baseClasses} {$variantClasses}"]) }}>
        ...
        <span class="flex items-center gap-2.5">
            ...
            @if ($icon)
                ...
                <x-aura::icon :name="$icon" :size="$iconSize" class="text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-200 transition-colors" />
        ...
        @if ($badge)
            ...
            <span class="{{ $badgeSizeClass }} font-mono px-1.5 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 font-semibold">
                {{ $badge }}
            </span>
