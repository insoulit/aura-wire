...
@php
    ...
    $variantClasses = match ($variant) {
        'primary', 'dark' => 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 border border-zinc-800 dark:border-zinc-200 shadow-2xs',
        ...
        default => 'bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-white shadow-2xs',
    };
    ...
    try {
        $svgHtml = function_exists('svg') ? svg($iconName, "shrink-0 {$svgSizeClasses}", $attributesToPass)->toHtml() : null;
    } catch (\Throwable $e) {
        $svgHtml = null;
    }
@endphp
...
@if ($hasContainer)
    <div {{ $attributes->merge(['class' => "{$containerSizeClasses} {$shapeClasses} {$variantClasses} inline-flex items-center justify-center shrink-0"]) }}>
        @if ($svgHtml)
            {!! $svgHtml !!}
        @else
            <svg class="shrink-0 {{ $svgSizeClasses }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
@else
    @if ($svgHtml)
        {!! $svgHtml !!}
    @else
        <svg class="shrink-0 {{ $svgSizeClasses }} {{ $extraClasses }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <circle cx="12" cy="12" r="9" stroke-width="2" />
        </svg>
    @endif
@endif
