...
@php
    ...
    $id = $id ?? $name ?? $modelName ?? ('checkbox_' . Str::random(8));
    $alignItems = $description ? 'items-start' : 'items-center';
@endphp

<label for="{{ $id }}" {{ $attributes->only('class')->merge(['class' => "flex {$alignItems} gap-2.5 cursor-pointer group select-none w-full " . ($disabled ? 'opacity-50 cursor-not-allowed' : '')]) }}>
